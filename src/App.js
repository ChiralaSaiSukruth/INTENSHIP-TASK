...
function App() {
  ...
  useEffect(() => {
    if (debouncedSearchData) {
      const filteredSuggestions = countryDetails
        .filter((data) =>
          data.country.toLowerCase().includes(debouncedSearchData.toLowerCase()) ||
          data.capital.toLowerCase().includes(debouncedSearchData.toLowerCase())
        )
  ...
  }, [debouncedSearchData, countryDetails]);

  const handleSuggestionClick = (suggestion) => {
    setSearchData(suggestion);
    const country = countryDetails.find((data) => data.country === suggestion);
  ...
  return (
    <div className="App">
      <h1 className='heading'>Country Search Engine</h1>
      ...
      {selectedCountry ? (
        ...
        debouncedSearchData && !selectedCountry && <p>No matching countries found.</p>
      )}
    </div>
  );
}
